  ...
  if(this.options.snap) {
    if(Object.isFunction(this.options.snap)) {
      ...
      if(Object.isArray(this.options.snap)) {
        ...
        p = p.map(function(v) {return (v/this.options.snap).round() * this.options.snap;}.bind(this));
      }
    }
  }

  if (this.options.superghosting)	{p[1] = point[1] - this.originalScrollTop;}
...
Draggable.prototype.initDrag = function(event) {
  ...
  if (Event.isLeftClick(event)) {
    ...
    if (tag_name==='INPUT' || tag_name==='SELECT' || tag_name==='OPTION' || tag_name==='BUTTON' || tag_name==='TEXTAREA') {return;}
